describe("GET call using environment variable", () => {
    it("Fetches data using a valid DID", () => {
      const did = Cypress.env("did") || "did:web:did.myn.social:JCfKwAdJEAsjQXKTUFQ27hQrxwxU5GJ4Sd9v6pBtEKdXvxJPh2GU"; // Default value if env var is not set
  
      cy.request({
        method: "GET",
         url: `${Cypress.config("baseUrlv2")}did/resolve-did-doc?did=${did}`,
      }).then((response) => {
